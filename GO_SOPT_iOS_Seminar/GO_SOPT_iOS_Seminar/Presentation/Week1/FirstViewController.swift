...
private extension FirstViewController {
    ...
    func pushToSecondViewController() {
        ...
        // 1. TextField의 text는  String? 타입이다. 이를 String 타입으로 변환하는 guard 문이다.
        //    만약 TextField 값이 nil이라면 함수 자체를 종료한다. - else {return }
        //    단, UIKIt 설계상 TextField는 값이 없어도 nil값을 반환하지 않는다. 대신 빈 문자열 ""을 반환한다.
        //    따라서 아래 guard 문은 항상 통과한다. (else문에 들어가지 않는 다는 뜻!)
        guard let name = nameTextField.text else { return }
        
        // -> 옵셔널만 벗기고 화면전환하면 오류는 발생하지 않는다.
        // But 나는 빈 문자열일때(사용자가 아무입력도 안하고 버튼을 눌렀을 때) 화면전환을 막고 싶어 아래 코드를 작성한다.
